...
Also callsigns.parquet (see extract_callsigns), consumed by build_legs.py to
stamp the flight number onto each leg. Its times are absolute instants, so they
are TIMESTAMP(ms, UTC) rather than a relative tick.
...
def extract_callsigns(d):
    """Callsign changes from the FULL-RATE trace, independent of the fit.

    Reading them off the stored CR nodes would be wrong: greedy node placement
    optimises curve fidelity, and the >=1 m dedup in build_nodes_cr discards
    stationary ground samples outright, so the node nearest a change can sit tens
    of seconds from it. This pass therefore works on st.parse output at full rate.
    It is O(n) and negligible next to the fit, and it does not touch the geometry
    output.

    Returns [(t, flight)], sparse -- one entry per change -- with t in SECONDS
    relative to the trace's `timestamp` base."""
    ms = st.parse(d)
...
def _work(path):
    try:
        ...
        # Callsign changes are stamped to ABSOLUTE instants here (epoch ms, stored
        # as TIMESTAMP): `t` in nodes.parquet stays int32-relative to this file's
        # own base, but these must be comparable across day-boundary releases.
        cs = extract_callsigns(d) or []
        ...
        return dict(icao=d.get("icao") or os.path.basename(path)[11:17],
                    ...
                    callsigns=[(epoch_ms(t), f) for (t, f) in cs])
